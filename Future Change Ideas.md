...
- [x] **Test Mode with Countdown**: Create a test mode for the user to practice writing essays under timed conditions. Timer must be placed close to the writing box. *(Completed: integrated dedicated countdown timer bar directly above the writing box `#stage2TimerBar`, quick toggle button in toolbar with `Alt+C` hotkey, duration presets [45m Cambridge exam standard, 40m, 30m, 15m], auto-start on candidate's first keystroke, visual progress bar, amber warning at 5m, critical pulse at 1m, zero-dependency Web Audio API alert chimes, Time's Up completion modal, and persistent exam metadata in history records)*
