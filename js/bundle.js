...
(function() {
  ...
  const INFORMAL_PATTERNS = [
    ...
    /\b(can't|don't|won't|isn't|aren't|didn't|doesn't|haven't|hasn't|hadn't)\b/i
  ];
  ...
  function checkTargetWordUsage(wordObj, text) {
    ...
    for (const stem of stems) {
      ...
      if (match) {
        return { used: true, matchedStem: stem, index: match.index };
      }
    }
  ...
  function analyzeQuickMetrics(text, targetVocabulary = []) {
    ...
    return {
      ...
      targetWordsUsed,
      targetWordsTotal: targetVocabulary.length,
      vocabStatus,
      detectedGrammar
    };
  }
  ...
  function evaluateEssay(text, currentTopic) {
    ...
    const targetUsageResults = targetVocabulary.map(v => {
      ...
      return { ...v, used: usage.used, matchedStem: usage.matchedStem };
    });
    ...
    ACADEMIC_C1_MARKERS.forEach(marker => {
      const matches = lowerText.match(new RegExp(`\\b${marker}\\b`, 'gi'));
    ...
    INFORMAL_PATTERNS.forEach(pat => {
      ...
      if (found) informalMatches.push(...found);
    });
    ...
    if (wordCount < 180) {
      contentScore -= 2.0;
      feedbackContent.push(`Length (${wordCount} words) is critically below Cambridge C1 recommendation (220-260 words). Arguments require substantive development.`);
    } else if (wordCount < 220) {
      contentScore -= 0.8;
      feedbackContent.push(`Slightly under the 220-word threshold (${wordCount} words). Expand with further analytical justification.`);
    } else if (wordCount > 340) {
      contentScore -= 0.5;
      feedbackContent.push(`Verbose (${wordCount} words). Cambridge examiners value concise, succinct expression.`);
    } else {
      feedbackContent.push(`Optimal word length (${wordCount} words) adhering strictly to Cambridge C1 guidelines.`);
    }

    if (paragraphs.length < 3) {
      contentScore -= 1.0;
      feedbackContent.push("Separate text clearly into Introduction, Body paragraphs (addressing both prompt points), and Conclusion.");
    } else {
      feedbackContent.push(`Strong architecture with ${paragraphs.length} distinct paragraphs.`);
    }
    ...
    if (informalMatches.length > 0) {
      ...
      commScore -= penalty;
      feedbackComm.push(`Detected informal phrasing or contractions ("${informalMatches.slice(0, 3).join(', ')}"). Avoid contractions and keep formal scholarly register.`);
    } else {
      commScore += 0.5;
      feedbackComm.push("Flawless formal academic register with zero informal contractions.");
    }

    if (identifiedStructures.some(s => s.id === 'passiveReporting' || s.id === 'cleftSentence')) {
      commScore += 0.5;
      feedbackComm.push("Effective use of academic hedging and cleft framing typical of Cambridge Band 5 essays.");
    }
    ...
    if (academicMarkerCount >= 4) {
      orgScore += 0.8;
      feedbackOrg.push(`Exceptional deployment of cohesive devices (${academicMarkerCount} advanced markers found).`);
    } else if (academicMarkerCount >= 2) {
      orgScore += 0.3;
      feedbackOrg.push("Adequate transitional markers; incorporate more concessive linkers (e.g. 'notwithstanding', 'inasmuch as').");
    } else {
      orgScore -= 0.8;
      feedbackOrg.push("Discourse cohesion underdeveloped. Integrate formal Cambridge transition linkers.");
    }
    ...
    if (vocabRatio >= 0.75) {
      langScore += 1.5;
      feedbackLang.push(`Outstanding command of Cambridge C1 topic vocabulary (${usedTargetCount}/${targetVocabulary.length} words integrated).`);
    } else if (vocabRatio >= 0.5) {
      langScore += 0.8;
      feedbackLang.push(`Good integration of target vocabulary (${usedTargetCount}/${targetVocabulary.length} words used), aim for at least 6.`);
    } else {
      feedbackLang.push(`Target vocabulary underutilized: only ${usedTargetCount}/${targetVocabulary.length} required words incorporated.`);
    }

    if (identifiedStructures.length >= 3) {
      langScore += 1.0;
      feedbackLang.push(`High grammatical ambition demonstrated (${identifiedStructures.length} advanced syntactic structures used).`);
    } else if (identifiedStructures.length >= 1) {
      langScore += 0.4;
      feedbackLang.push(`Incorporated advanced syntax (${identifiedStructures[0].name}). Add negative inversion or inverted conditionals for C2.`);
    } else {
      langScore -= 0.5;
      feedbackLang.push("Syntax is basic/compound. Cambridge C1 requires varied complex structures (inversions, clefts).");
    }

    if (typeTokenRatio > 0.55 && wordCount > 150) langScore += 0.3;
    langScore = Math.max(1, Math.min(5, langScore));

    const rawTotal = contentScore + commScore + orgScore + langScore;
  ...
  function calculateWordSimilarity(s1, s2) {
    ...
    if (s1.startsWith(s2) || s2.startsWith(s1)) {
      if (Math.abs(s1.length - s2.length) <= 3) return 0.88;
    }
  ...
  class CambridgeSpeechEngine {
    ...
    initRecognition() {
      ...
      this.recognition.onresult = (event) => {
        ...
        if (activeText) this.processSpokenSpeech(activeText);
      };

      this.recognition.onerror = (event) => {
        if (event.error === 'not-allowed') {
          if (this.onError) this.onError("Microphone permission denied. Allow mic access to evaluate speech.");
        }
        this.stopListening();
      };
    ...
    async startListening(canvasElement) {
      if (!this.isSpeechSupported()) {
        if (this.onError) this.onError("Speech Recognition is not supported by your browser. Try Chrome, Edge, or Safari.");
        return;
      }
      try {
        this.initRecognition();
        this.recognition.start();
        if (canvasElement) await this.startAudioVisualizer(canvasElement);
      } catch (err) {
        if (this.onError) this.onError("Microphone error: " + err.message);
      }
    }
    ...
    getFinalSpeakingAssessment() {
      ...
      if (pronunciationAccuracy >= 90) {
        ...
        pronunciationFeedback.push("Exceptional phonological precision and phonemic clarity across academic lexis.");
        ...
        pronunciationFeedback.push("Noticeable accent interference on complex C1 multisyllabic terms. Practice word stress.");
      } else {
      ...
      if (wpm >= 130 && wpm <= 165) {
        fluencyScore = 5.0;
        fluencyFeedback.push(`Optimal Cambridge native-speed pacing at ${wpm} WPM with confident, steady cadence.`);
      } else if ((wpm >= 110 && wpm < 130) || (wpm > 165 && wpm <= 185)) {
        fluencyScore = 4.0;
        fluencyFeedback.push(`Acceptable speaking rate (${wpm} WPM). Aim for consistent 135-150 WPM rhythm.`);
      } else if (wpm < 110) {
        fluencyScore = 3.0;
        fluencyFeedback.push(`Hesitant pace (${wpm} WPM). Work on smooth transitional phrasing to minimize unnatural pauses.`);
      } else {
        fluencyScore = 3.5;
        fluencyFeedback.push(`Rushed pace (${wpm} WPM). Slow down slightly to emphasize rhetorical stress.`);
      }
      ...
      if (readRatio >= 0.90) {
        ...
        discourseScore = 3.5;
        discourseFeedback.push(`Covered ${Math.round(readRatio * 100)}% of text.`);
      }
      ...
      return {
        ...
        meetsC1Speaking,
        scores: {
          pronunciation: { score: Number(pronunciationScore.toFixed(1)), feedback: pronunciationFeedback },
          fluency: { score: Number(fluencyScore.toFixed(1)), feedback: fluencyFeedback },
          discourse: { score: Number(discourseScore.toFixed(1)), feedback: discourseFeedback }
        }
      };
    }
  ...
  class CambridgeApp {
    ...
    loadTopic(index) {
      ...
      this.dom.topicPoints.innerHTML = this.currentTopic.prompt.points.map(p => `<li>${p}</li>`).join('');
    ...
    pickRandomTopic() {
      ...
      this.showToast(`Loaded topic: "${this.currentTopic.title.slice(0, 32)}..."`, "info");
    }

    renderVocabularyChips() {
      const text = this.dom.essayInput.value || "";
      this.dom.vocabGrid.innerHTML = this.currentTopic.targetVocabulary.map((v) => {
        const usage = checkTargetWordUsage(v, text);
        return `
    ...
    handleEditorInput() {
      ...
      metrics.vocabStatus.forEach(status => {
        ...
        if (chip) {
          if (status.used) chip.classList.add('used');
    ...
    toggleExamTimer() {
      if (this.isTimerRunning) this.pauseExamTimer();
      else this.startExamTimer();
    }
    ...
    openHistoryDrawer() { this.dom.historyDrawer.classList.add('open'); }
    closeHistoryDrawer() { this.dom.historyDrawer.classList.remove('open'); }
